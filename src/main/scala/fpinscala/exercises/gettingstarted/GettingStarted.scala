...
object PolymorphicFunctions:
  ...
  def findFirst[A](as: Array[A], p: A => Boolean): Int =
    //p: equality function
    //as: array of generic type A
    //return: index of first element in array that satisfies p

    @annotation.tailrec
    def loop(n: Int): Int =
      if n >= as.length then -1 //out of bounds
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean =
    ...
    def loop (array: Array[A], index: Int): Boolean = {
      ...
      if index == array.length - 1 then true
      //if the array is not sorted
      else if gt(array(index), array(index + 1)) then false
      //if the array is sorted
      else loop(array, index + 1)
    }

    loop(as, 0)

  def isSortedWithMatching[A](as: Array[A], gt: (A, A) => Boolean): Boolean =

    def loop(list: List[A]): Boolean = list match {
      case Nil => true // The list is empty, so it's sorted
      case _ :: Nil => true // Single element list is always sorted
      case x :: y :: xs =>
        if gt(x, y) then false // The list is not sorted
        else loop(y :: xs) // Continue checking the rest of the list
    }
